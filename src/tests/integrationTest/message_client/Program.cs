...
var queueList = new TopicSpecification[]{
    new TopicSpecification()
    {
        Name = GetEnvironmentVariable("BALANCEWORKER_QUEUE"),
        NumPartitions = 1
    },
    new TopicSpecification()
    {
        Name = GetEnvironmentVariable("FIBONACCI_QUEUE"),
        NumPartitions = 4
    },
    new TopicSpecification()
    {
        Name = GetEnvironmentVariable("LONGRUNNINGBATCHTASK_QUEUE"),
        NumPartitions = 4
    },
};
...
{
    case "rabbitmq":
        RabbitMqInit(queueList.Select(x=>x.Name).ToArray());
        break;
    case "kafka":
        await KafkaInit(queueList).ConfigureAwait(false);
        break;
    default:
