...
namespace MessageWorkerPool.RabbitMq
{
    ...
    public class RabbitMqWorker : IWorker
    {
        ...
        /// <summary>
        /// _pipeDataStream: sending in message that get from MQ.
        /// _pipeDataStream: MESSAGE_DONE or MESSAGE_DONE_WITH_REPLY = Finish task, we can do BasicAck, otherwise will wait for signal that we can ack.
        /// </summary>
        ...
        public async Task GracefulShutDownAsync(CancellationToken token)
        {
            using (Logger.BeginScope($"[Pid: {Process.Id}]"))
            {
                ...
                //Logger.LogInformation($"[workerOperationPipe]sending MessageCommunicate.CLOSED_SIGNAL");
                //await _pipeDataStream.WriteObjectAsync(MessageCommunicate.CLOSED_SIGNAL).ConfigureAwait(false);
                //Logger.LogInformation($"[workerOperationPipe]closed MessageCommunicate.CLOSED_SIGNAL");
                await CloseProcess();
